...
Java_com_example_calc24_Calc24Activity_calc24(
        JNIEnv* env,
        jobject /* this */,
        jintArray nums) {

    std::vector<CalcItem> vec;
    int *numArray = env->GetIntArrayElements(nums, nullptr);
    for (int i = 0; i < 4; i++) {
        vec.emplace_back(numArray[i]);
    }
    env->ReleaseIntArrayElements(nums, numArray, 0);
